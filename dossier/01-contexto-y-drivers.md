...
- **Técnica:** `service_role` solo en `*.server.ts` — desde el 15/09/2026 lo garantiza una fitness function que corre en CI en cada PR (`src/architecture/service-role-boundary.test.ts`), verificada en vivo con una violación deliberada.
...
**Acceso actualizado el 18/09/2026:** el equipo confirmó que el tercer integrante ya tiene acceso al proyecto. La revisión previa mostró dos cuentas con rol `Owner` y MFA deshabilitado; por ello, la continuidad de acceso mejora, pero todavía corresponde aplicar privilegios mínimos y habilitar MFA en las cuentas administrativas.

**Respaldo y restauración verificados el 18/09/2026:** se actualizó Docker Desktop a 4.91.0, se generó un respaldo lógico y se restauró en un Supabase local desechable. Coincidieron los conteos de las 14 tablas públicas y los 4 usuarios de Auth. La base remota no fue modificada y el respaldo sensible temporal no se incorporó a Git. Ver `experimentos/EXP-003-recuperacion/README.md`.
